...
def render_admin_usuarios():
    ...
    # =========================
    # 2. USUÁRIOS ATIVOS (EDITE E SALVE)
    # =========================
    ...
    edited = st.data_editor(
        ...
        column_config={
            ...
            "access_key": None,  # Oculta a chave de acesso da tabela
    ...
    with st.expander("Visualizar Chave de Acesso"):
        st.write("Confirme sua própria chave de acesso para visualizar a chave de outro usuário.")
        options_view = [f'{int(r.id)} • {r.nome} • {r.email}' for r in df.itertuples(index=False)]
        picked_view = st.selectbox("Selecione o usuário", options_view, key="u_view_pick")
        admin_key = st.text_input("Sua Chave de Acesso", type="password", key="admin_key_view")
        
        if st.button("Ver Chave", key="v_btn"):
            if admin_key == st.session_state.get("access_key"):
                picked_id_v = int(picked_view.split("•")[0].strip())
                target_row = df.loc[df["id"] == picked_id_v]
                if not target_row.empty:
                    target_key = target_row.iloc[0]["access_key"]
                    st.success("Autenticação bem-sucedida.")
                    st.info("Chave de acesso selecionada:")
                    st.code(target_key)
            else:
    ...
    # =========================
    with st.expander("Histórico de login (últimos 200)"):
